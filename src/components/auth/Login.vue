<template>
  <v-flex v-show="!isLogin" xs12 sm6 md8 lg8 xl4 offset-sm3 offset-md2 offset-lg2 offset-xl4 align-center align-content-center class="pt-2">
    <v-card hover class="pa-5">
        ...
        <v-card-title primary-title>
            <div class="full-width text-xs-center">
                <h3 class=" headline mb-2 ">Login</h3>
            </div>
        </v-card-title>
    <v-alert color="error"  transition="fade-transition" v-show="error" icon="warning" value="true" dismissible v-model="alert" v-on:click="error = false">
    ...
    <div>
        <v-btn color="primary" large @click="login" :loading="loading" :disabled="disable">Login</v-btn>
    </div>
    </v-card>
  </v-flex>
</template>
